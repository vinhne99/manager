...
class Media extends CI_Controller {
	...
	public function upload_tour()
	{
		...
		)
		{
			...
			if(move_uploaded_file($_FILES["uploadfile"]["tmp_name"],$upload_path.$filename))
			{
				...
				$img_id = $this->image_model->insert(array('parent_id' => $id , 'image_path' => $filename , 'date_create' => date_now()));
	...
	public function upload_post(){
		$upload_path = 'uploads/images/post/';
		if(!is_dir($upload_path)){ mkdir($upload_path); }
		if (
		(
			($_FILES["uploadfile"]["type"] == "image/gif") ||
		...
		)
		{
			...
			if(move_uploaded_file($_FILES["uploadfile"]["tmp_name"],$upload_path.$filename))
			{
				...
				$size_img = $this->config->item('size_img_post');
				foreach ($size_img as $img){
					if ($img[0] == '' && $img[1] == '') {
						$this->simpleimage->load($upload_path . $filename)->save($upload_path . $filename);
					} else {
						$upload_path_size = $upload_path . '/' . $img[0] . '/';
						if (!is_dir($upload_path_size)) {
							mkdir($upload_path_size);
						}
						if (isset($img[2])) {
							$this->simpleimage->load($upload_path . $filename)->best_fit($img[0], $img[1])->save($upload_path_size . $filename);
						} else {
